...
{
    return ((s->intr & s->inte) | s->intf) & TIMER_ALARM_MASK;
}
...
static uint64_t rp2040_timer_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    qemu_log_mask(LOG_UNIMP, "rp2040.timer: read  "
                  "(size %d, addr 0x%08" HWADDR_PRIx
                  ", offset 0x%04" HWADDR_PRIx ") -> 0x%0*" PRIx64 "\n",
                  size, RP2040_TIMER_BASE + addr, offset, size << 1, value);
    return value;
}
                               ...
                               uint64_t value64, unsigned size)
{
    ...
    qemu_log_mask(LOG_UNIMP, "rp2040.timer: write "
                  "(size %d, addr 0x%08" HWADDR_PRIx
