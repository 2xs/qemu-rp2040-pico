...
static uint64_t rp2040_xosc_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    qemu_log_mask(LOG_UNIMP, "rp2040.xosc: read  "
                  "(size %d, addr 0x%08" HWADDR_PRIx
                  ", offset 0x%04" HWADDR_PRIx ") -> 0x%0*" PRIx64 "\n",
                  size, RP2040_XOSC_BASE + addr, offset, size << 1, value);
    return value;
}
                              ...
                              uint64_t value64, unsigned size)
{
    ...
    qemu_log_mask(LOG_UNIMP, "rp2040.xosc: write "
                  "(size %d, addr 0x%08" HWADDR_PRIx
                  ", offset 0x%04" HWADDR_PRIx
                  ", value 0x%0*" PRIx64 ")\n",
                  size, RP2040_XOSC_BASE + addr, offset, size << 1, value64);
}
